...
- (void)renderTileViews
{
    ...
    for (int  y = 0; y < _numberOfRows; y++) {
        ...
        for (int x = 0; x < _numberOfColumns; x++) {
            ...
            if (y && y != _numberOfRows - 1 && x && x != _numberOfColumns - 1) {
...
- (void)startAnimatingWithBeginTime:(NSTimeInterval)beginTime {
    ...
    for (NSArray *tileRows in _tileViewRows) {
        for (TileView *view in tileRows) {
            ...
            vector = CGPointMake(vector.x * _kRippleDelayMultiplier * distance, vector.y * kRippleMagnitudeMultiplier * distance);
            
            [view startAnimatingWithDuration:kAnimationDuration beginTime:beginTime rippleDelay:distance * _kRippleDelayMultiplier rippleOffset:vector];
        }
    }
